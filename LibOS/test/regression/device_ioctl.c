...
struct gramine_ioctl_read {
    size_t buf_size;        /* in */
    char* buf;              /* out */
    int64_t off;            /* in/out -- updated after read */
...
int main(int argc, char* arvg[]) {
    ...
    off_t offset = ioctl(devfd, GRAMINE_IOCTL_REWIND);
    if (offset < 0)
        err(1, "/dev/gramine_test_dev ioctl(GRAMINE_IOCTL_REWIND)");
    if (offset > 0)
        errx(1, "/dev/gramine_test_dev ioctl(GRAMINE_IOCTL_REWIND) didn't return 0 "
                "(returned: %ld)", offset);
    ...
    ssize_t devfd_size = ioctl(devfd, GRAMINE_IOCTL_GETSIZE);
    if (devfd_size < 0)
        err(1, "/dev/gramine_test_dev ioctl(GRAMINE_IOCTL_GETSIZE)");
    if (devfd_size != sizeof(STRING_READWRITE))
        errx(1, "/dev/gramine_test_dev ioctl(GRAMINE_IOCTL_GETSIZE) didn't return %lu "
                "(returned: %ld)", sizeof(STRING_READWRITE), devfd_size);

    /* test 2 -- use ioctl(GRAMINE_IOCTL_WRITE) and ioctl(GRAMINE_IOCTL_READ) syscalls */
    ret = ioctl(devfd, GRAMINE_IOCTL_CLEAR);
    if (ret < 0)
        err(1, "/dev/gramine_test_dev ioctl(GRAMINE_IOCTL_CLEAR)");

    struct gramine_ioctl_write write_arg = {
        .buf_size = sizeof(STRING_IOCTL),
        .buf      = STRING_IOCTL,
        .off      = 0 };
    ret = ioctl(devfd, GRAMINE_IOCTL_WRITE, &write_arg);
    if (ret < 0)
        err(1, "/dev/gramine_test_dev ioctl(GRAMINE_IOCTL_WRITE)");
    if (write_arg.off != sizeof(STRING_IOCTL))
        errx(1, "/dev/gramine_test_dev ioctl(GRAMINE_IOCTL_WRITE) didn't update offset to %lu "
                "(returned: %ld)", sizeof(STRING_IOCTL), write_arg.off);
    if (write_arg.copied != sizeof(STRING_IOCTL))
        errx(1, "/dev/gramine_test_dev ioctl(GRAMINE_IOCTL_WRITE) didn't copy %lu bytes "
                "(returned: %ld)", sizeof(STRING_IOCTL), write_arg.copied);

    memset(buf, 0, sizeof(buf));
    struct gramine_ioctl_read read_arg = {
        .buf_size = sizeof(buf) - 1,
        .buf      = buf,
        .off      = 0 };
    ret = ioctl(devfd, GRAMINE_IOCTL_READ, &read_arg);
    if (ret < 0)
        err(1, "/dev/gramine_test_dev ioctl(GRAMINE_IOCTL_READ)");
    if (read_arg.off != sizeof(STRING_IOCTL))
        errx(1, "/dev/gramine_test_dev ioctl(GRAMINE_IOCTL_READ) didn't update offset to %lu "
                "(returned: %ld)", sizeof(STRING_IOCTL), read_arg.off);
    if (read_arg.copied != sizeof(STRING_IOCTL))
        errx(1, "/dev/gramine_test_dev ioctl(GRAMINE_IOCTL_READ) didn't copy %lu bytes "
    ...
    devfd_size = ioctl(devfd, GRAMINE_IOCTL_GETSIZE);
    if (devfd_size < 0)
        err(1, "/dev/gramine_test_dev ioctl(GRAMINE_IOCTL_GETSIZE)");
    ...
    /* test 3 -- use complex ioctl(GRAMINE_IOCTL_REPLACE_ARR) syscall */
	struct gramine_ioctl_replace_char replace_chars[] = {
        { .src = 'l', .dst = '$' },
        { .src = 'o', .dst = '0' }
    };
	struct gramine_ioctl_replace_arr replace_arr = {
        .replacements_cnt = 2,
        .replacements_arr = replace_chars
    };
    ret = ioctl(devfd, GRAMINE_IOCTL_REPLACE_ARR, &replace_arr);
    if (ret < 0)
        err(1, "/dev/gramine_test_dev ioctl(GRAMINE_IOCTL_REPLACE_ARR)");
    ...
    /* test 4 -- use complex ioctl(GRAMINE_IOCTL_REPLACE_LIST) syscall */
	struct gramine_ioctl_replace_char replace_char_1 = { .src = '$', .dst = 'l', .next = NULL };
	struct gramine_ioctl_replace_char replace_char_2 = { .src = '0', .dst = 'o',
                                                         .next = &replace_char_1 };
	struct gramine_ioctl_replace_list replace_list = { .replacements_list = &replace_char_2 };

    ret = ioctl(devfd, GRAMINE_IOCTL_REPLACE_LIST, &replace_list);
    if (ret < 0)
        err(1, "/dev/gramine_test_dev ioctl(GRAMINE_IOCTL_REPLACE_LIST)");
