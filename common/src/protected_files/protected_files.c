...
static bool ipf_read_node(pf_context_t* pf, uint64_t node_number, void** buffer,
                          uint32_t node_size) {
    pf_status_t status = g_cb_read(pf->file, buffer, node_number * node_size, node_size,
                                   pf->real_file_size, &pf->addr);
    if (PF_FAILURE(status)) {
        pf->last_error = status;
        return false;
    }
...
static bool ipf_write_node(pf_context_t* pf, uint64_t node_number, const void* buffer,
                           uint32_t node_size) {
    pf_status_t status = g_cb_write(pf->file, buffer, node_number * node_size, node_size,
                                    &pf->real_file_size, &pf->addr);
    if (PF_FAILURE(status)) {
        pf->last_error = status;
        return false;
...
static bool ipf_update_all_data_and_mht_nodes(pf_context_t* pf) {
    ...
    pf_status_t status;
    encrypted_node_t temp_enc_node;
    void* data = lruc_get_first(pf->cache);

    // find the node with largest node number in LRU cache
    uint64_t max_node_number = 1;
    file_node_t* max_node = NULL;
    while (data != NULL) {
        if (((file_node_t*)data)->type == FILE_DATA_NODE_TYPE) {
            file_node_t* data_node = (file_node_t*)data;
            if (data_node->need_writing && data_node->physical_node_number > max_node_number) {
                max_node_number = data_node->physical_node_number;
                max_node = data_node;
            }
        }
    ...
    // Flush the node with max node number first to avoid frequent remaps
    if (max_node != NULL) {
        gcm_crypto_data_t* max_gcm_crypto_data = 
            &max_node->parent->decrypted.mht
                .data_nodes_crypto[max_node->node_number % ATTACHED_DATA_NODES_COUNT];

        if (!ipf_generate_random_key(pf, &max_gcm_crypto_data->key))
            goto out;

        status = g_cb_aes_gcm_encrypt(&max_gcm_crypto_data->key, &g_empty_iv, NULL, 0,
                                      max_node->decrypted.data.data, PF_NODE_SIZE,
                                      temp_enc_node.cipher, &max_gcm_crypto_data->gmac);
        if (PF_FAILURE(status)) {
            pf->last_error = status;
            goto out;
        }

        if (!ipf_write_node(pf, max_node->physical_node_number, &temp_enc_node, PF_NODE_SIZE))
            goto out;

        max_node->need_writing = false;
        max_node->new_node = false;
    }
        ...
        if (((file_node_t*)data)->type == FILE_DATA_NODE_TYPE) {
            ...
            if (data_node->need_writing) {
                ...
                status = g_cb_aes_gcm_encrypt(&gcm_crypto_data->key, &g_empty_iv, NULL, 0,  // aad
                                              ...
                                              temp_enc_node.cipher, &gcm_crypto_data->gmac);
                if (PF_FAILURE(status)) {
                    pf->last_error = status;
                    goto out;
                }

                if (!ipf_write_node(pf, data_node->physical_node_number, &temp_enc_node,
                                    PF_NODE_SIZE))
                    goto out;
    ...
    for (dirty_idx = dirty_count; dirty_idx > 0; dirty_idx--) {
        ...
        if (!ipf_write_node(pf, file_mht_node->physical_node_number, &temp_enc_node, PF_NODE_SIZE))
            goto out;
    ...
    status = g_cb_aes_gcm_encrypt(&pf->encrypted_part_plain.mht_key, &g_empty_iv, NULL, 0,
                                  ...
                                  temp_enc_node.cipher, &pf->encrypted_part_plain.mht_gmac);
    if (PF_FAILURE(status)) {
        pf->last_error = status;
        goto out;
    }

    if (!ipf_write_node(pf, pf->root_mht.physical_node_number, &temp_enc_node, PF_NODE_SIZE))
        goto out;
...
out:
    erase_memory(&temp_enc_node, sizeof(temp_enc_node));
    free(mht_array);
    return ret;
}
...
static bool ipf_update_metadata_node(pf_context_t* pf) {
    ...
    if (!ipf_write_node(pf, /*node_number=*/0, &pf->file_metadata, PF_NODE_SIZE))
        return false;
...
static file_node_t* ipf_read_data_node(pf_context_t* pf, uint64_t offset) {
    ...
    void* file_data_node_addr = file_data_node->encrypted.cipher;
    if (!ipf_read_node(pf, file_data_node->physical_node_number, &file_data_node_addr,
                       PF_NODE_SIZE)) {
        free(file_data_node);
        return NULL;
    }
...
static file_node_t* ipf_read_mht_node(pf_context_t* pf, uint64_t mht_node_number) {
    ...
    void* file_mht_node_addr = file_mht_node->encrypted.cipher;
    if (!ipf_read_node(pf, file_mht_node->physical_node_number, &file_mht_node_addr,
                       PF_NODE_SIZE)) {
        free(file_mht_node);
        return NULL;
    }
...
static bool ipf_init_existing_file(pf_context_t* pf, const char* path) {
    ...
    // read meta-data node
    metadata_node_t* file_metadata_addr = &pf->file_metadata;
    if (!ipf_read_node(pf, /*node_number=*/0, &file_metadata_addr, PF_NODE_SIZE))
        return false;

    // if callback uses mmap, then pointer is changed to the mapped address
    if (file_metadata_addr == pf->addr)
        memcpy(&pf->file_metadata, file_metadata_addr, PF_NODE_SIZE);
    ...
    if (pf->encrypted_part_plain.size > MD_USER_DATA_SIZE) {
        ...
        void* root_mht_addr = pf->root_mht.encrypted.cipher;
        if (!ipf_read_node(pf, /*node_number=*/1, root_mht_addr, PF_NODE_SIZE));
            return false;
